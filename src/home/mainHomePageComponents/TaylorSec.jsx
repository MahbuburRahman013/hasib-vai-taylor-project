                ...
                data.map((item, index) => <div className='mb-20' key={index}>
                    <div className={` flex md:flex-row flex-col md:gap-y-0 gap-y-5   ${index === 1 && 'md:flex-row-reverse'} justify-between`}>
                        <div className='md:w-[50%] w-full lg:p-12 px-2'>
                            <h1 className='text-3xl text-[#090d82] font-normal hover:underline'>{item.title}</h1>
                            <p className='lg:text-xl text-base mt-5 mb-12 text-[#121333] '>Provides access to more than 2,700 high-quality, cross-disciplinary journals spanning Humanities and Social Sciences, Science and Technology, Engineering, Medicine and Healthcare.</p>
                            <div className='flex justify-center'><button><MdOutlineArrowCircleRight className=' text-[#090d82] text-4xl font-semibold' /></button></div>
                        </div>
                        <div className={`md:w-[50%] w-full  ${index == 0 ? 'bg-[#006db4]' : index == 1 ? 'bg-[#007a96]' : 'bg-[#f2673c]'} p-3`}>
